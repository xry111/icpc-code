...
\lstinputlisting[language=c++]{hungary.cc}

\subsubsection{Hopcraft-Karp}

时间复杂度$\mathcal{O}(\sqrt{V}E)$。
\lstinputlisting[language=c++]{hopcraft-karp.cc}

\subsection{二分图最大权匹配}

时间复杂度 $\mathcal{O}(V^3)$

\lstinputlisting[language=c++]{kuhn-munkres.cc}

\subsection{一般图最大权匹配}

时间复杂度 $\mathcal{O}(V^3)$

\lstinputlisting[language=c++]{weighted-blossom-tree.cc}

\subsection{最大流}

\subsubsection{Highest Label Push-Relabel}
时间复杂度$\mathcal{O}(V^2 \sqrt{E})$。
\lstinputlisting[language=c++]{highest-label-push-relabel.cc}

\subsubsection{Dinic}
时间复杂度$\mathcal{O}(V^2E)$。
\lstinputlisting[language=c++]{dinic.cc}
...
时间复杂度 $\mathcal{O}(fVE)$。
\lstinputlisting[language=c++]{edmonds-karp.cc}
